...
class Validator {
  ...
  private validates(
    ...
    const errorElement = parentElement?.querySelector<HTMLInputElement>(
    ...
    for (const check of rules) {
      switch (inputElement.type) {
        ...
        case "checkbox":
          errorMessage = check(
            document.querySelector<HTMLInputElement>(`${rule.selector}:checked`)
              ?.value
          );
          break;
        default:
      ...
      formElement.onsubmit = (e) => {
        ...
        if (isFormValid) {
          if (typeof this.option.onSubmit === "function") {
            const enableInputs =
              formElement.querySelectorAll<HTMLInputElement>("[name]");
    ...
    return {
      ...
      check: (value: string) => {
        ...
        return emailRegex.test(value)
          ? undefined
          : message || `Truong nay khong phai email address`;
      },
    };
  }
    ...
    return {
      ...
      check: (value: string) => {
        ...
        return valueLength > minLength
          ? undefined
          : message || `Vui long nhap hon ${minLength} ky tu`;
      },
    };
  }
  static isConfirm(
    ...
    return {
      ...
      check: (value) => {
        return value.length < maxLength
          ? undefined
          : message || `Vui long nhap nho hon ${maxLength}`;
      },
    };
  }
    ...
    return {
      ...
      check: (value: string) => {
        return Number(value) > Number(minValue)
          ? undefined
          : message || `Vui long nhap so lon hon ${minValue}`;
      },
    };
  }
  static isLessThan(
    ...
    return {
      ...
      check: (value: string) => {
        return Number(value) < Number(maxValue)
          ? undefined
          : message || `Vui long nhap so nho hon ${maxValue}`;
      },
    };
  }
  static isInRange(
    ...
    return {
      ...
      check: (value: string) => {
        return Number(value) >= Number(minValue) &&
          ...
          ? undefined
          : message || `Vui long nhap so trong khoang ${minValue}-${maxValue}`;
      },
    };
  }
}
